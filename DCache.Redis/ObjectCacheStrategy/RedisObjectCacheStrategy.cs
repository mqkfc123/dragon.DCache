...
using DCache.Redis.ContainerCacheStrategy;
...
namespace DCache.Redis.ObjectCacheStrategy
{
    /// <summary>
    ...
    //where TContainerBag : class, IBaseContainerBag, new()
    {
        ...
        /// <summary>
        /// 若一个类中有静态构造函数，在首次实例化该类或任何的静态成员被引用时，
        /// .NET自动调用静态构造函数来初始化该类
        /// </summary>
        ...
        protected HashKeyAndField GetHashKeyAndField(string key, bool isFullKey = false)
        {
            ...
            {
                Key = finalFullKey,//finalFullKey.Substring(0, index),
        ...
        /// </summary>
        /// <param name="key"></param>
        /// <param name="tateTime"></param>
        public void SetKeyExpire(string key, DateTime tateTime)
        {
            if (string.IsNullOrEmpty(key))
            {
                return;
            ...
            var hashKeyAndField = this.GetHashKeyAndField(key);
            _cache.KeyExpire(hashKeyAndField.Key, tateTime); 
        }
